...
@onready var current_segments = null
@onready var lure_spawned = false
...
func spawnRope():
	...
	var rope_end = rope_end_scene.find_child("bob",true,false)
	rope_end.reparent(Bob)
	end_pin_joint = rope_end.find_child("PinJoint2D")
	rope_end.global_position = player.find_child("Line", true,false).global_position
	lure_spawned = true

func ropeOnScreen():
	print("ropeonescreen")
	var base_interval = 5
	var rope_end = find_child("bob", true, false)
	var rope_end_joint = rope_end.find_child("PinJoint2D")
	var distance = rope_end.global_position.distance_to(player.find_child("Line",true,false).global_position)
	...
	var rope_starting_position = player.find_child("Line",true,false).global_position
	...
	player.find_child("Line",true,false).find_child("PinJoint2D",true,false).set_node_b("")
	rope_segments.remove_at(-1)
	...
	current_segments = number_of_segments
	rope_spawned = true
	lure_spawned = false
...
func addRopeSegment(latest_segment, i, rotation_angle, pos):
	var pinJoint = latest_segment.find_child("PinJoint2D")
	...
	pinJoint.set_node_a(latest_segment.get_path())
...
func _process(delta: float) -> void:
	...
	if rope_spawned == true:
		update_line_2d_rope()
	if lure_spawned == true:
		print(find_child("bob",true,false).get_colliding_bodies())
		if find_child("bob",true,false).get_contact_count() > 0:
			ropeOnScreen()
